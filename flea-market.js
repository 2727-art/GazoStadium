  ...
  return renderFrame(`<section class="flea-sell" aria-labelledby="fleaSellHeading">
    ...
    <div class="flea-editor-layout">
      ...
      <form class="flea-form" id="fleaListingForm" aria-busy="${Boolean(state.busyAction)}">
        ...
        <section class="flea-x-field"><div class="flea-field-head"><label for="fleaXPostUrl">参考にするXポスト（任意）</label><small>個別ポストのみ</small></div><input id="fleaXPostUrl" name="xPostUrl" type="url" inputmode="url" maxlength="240" value="${escapeHtml(draft.xPostUrl)}" placeholder="https://x.com/username/status/..." autocomplete="off" autocapitalize="none" spellcheck="false" aria-describedby="fleaXPostHint" /><small id="fleaXPostHint">自分のポストの個別URLを指定できます。ゲーム内にはポスト本文・画像・プレビューを表示せず、外部リンクだけを置きます。</small>
          <label class="flea-consent"><input id="fleaXConsent" name="xConsent" type="checkbox" ${draft.xConsent ? "checked" : ""} /><span>自分の投稿で、必要な権利・写っている人の公開同意があり、成人向け・個人情報・実物/外部取引を含まないことを確認し、外部リンクの公開に同意します</span></label></section>
