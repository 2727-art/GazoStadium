...
test("oshi activity collection has the agreed titles and only reuses valid shared products", () => {
  ...
  assert.match(cleanup, /targetState\.pendingOffer = null/);
  assert.match(cleanup, /targetState\.pendingIncomingOffer = null/);
...
test("market shop charms use owned stamp IDs without consuming chat equipment slots", () => {
  ...
  for (const entry of ["online.js", "strategy.js", "market.js", "flea-market.js"]) {
    assert.match(read(entry), /player-titles\.js\?v=player-titles-v3/, entry);
  }
});
