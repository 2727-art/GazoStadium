...
test("new client and Functions agree on the protected protocol and variant", () => {
  ...
  assert.match(functions, /finalizeServerRankingAwards\(uid, now\)/);
  assert.match(functions, /teamEligibilityArchive/);
...
test("database rules keep legacy rooms while sealing qualification and scores", () => {
  const rules = JSON.parse(read("database.rules.json")).rules.online;
  assert.equal(rules.teamEligibility.$uid[".write"], false);
  assert.equal(rules.teamEligibilityArchive[".read"], false);
  assert.match(rules.teamEligibility.$uid[".read"], /auth\.uid === \$uid/);
  assert.match(rules.teamQueue.$uid[".validate"], /protocolVersion/);
  assert.match(rules.teamQueue.$uid[".validate"], /oshi_jouzu_duo/);
  assert.match(rules.teamQueue.$uid[".validate"], /teamEligibility/);
  assert.equal(rules.teamRooms.$roomId.protocolVersion[".validate"], "newData.val() === 2");
  assert.match(rules.teamRooms.$roomId[".write"], /roster/);
  assert.match(rules.teamRooms.$roomId.status[".write"], /accepted/);
