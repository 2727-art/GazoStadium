...
const MARKER = "rate-floor-showcase-v1";
const ASSETS = ["app.js", "online.js", "styles.css"];
const EXCLUDED = ["/functions/scripts/sync-rate-floor-showcase.js", "/functions/docs/RATE_FLOOR_SHOWCASE_RELEASE.md"];
...
async function main() {
  ...
  await fs.writeFile(path.join(ROOT, "functions/docs/RATE_FLOOR_SHOWCASE_RELEASE_QA.json"), `${JSON.stringify(report, null, 2)}\n`);
